...
var _check = function(original, auth, cb) {
    var str = original;
    yaspeller.checkText(str, function(err, resp) {
        for (var i = 0; i < resp.length; i++) {
            if (resp[i].s[0] != undefined && auth.indexOf(resp[i].word.toLowerCase()) == -1) {
...
var _spellChecker = {
    check: function(str, authorized, cb){
        _check(str, authorized, function(resp){
            cb(resp);
        });
    }
}
